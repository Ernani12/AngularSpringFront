...
// Método para manejar el evento de clic en el mapa
  ...
  loadGoogleMapsScript(callback: () => void): void {
    ...
    script.src = `https://maps.googleapis.com/maps/api/js?key=AIzaSyAGFWBA1FqUs9EwRotXsehq6aDr_rD0Zxo&loading=async&callback=initMap`;
    script.async = true;
    script.defer = true;
  ...
  initializeMap(): void {
    ...
    this.map.addListener('click', (event: google.maps.MapMouseEvent) => {
      if (event.latLng) {
        ...
        console.log(`Coordenadas atualizadas: ${this.lat}, ${this.lng}`);
  ...
  updateMarkerPosition(): void {
    const newPosition = new google.maps.LatLng(this.lat, this.lng);
    if (this.marker) {
  ...
  getGeocoding(lat: number, lng: number): void {
    ...
    geocoder.geocode({ location: latlng }, (results, status) => {
      if (status === 'OK') {
        ...
        this.address = 'Geocoder failed: ' + status;
      }
    });
  }
